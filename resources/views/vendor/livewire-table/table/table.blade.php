<table class="relative w-full" x-data="{ selected: @entangle('selected') }">
    <thead class="border-b border-neutral-200 dark:border-neutral-700">
    <tr class="group">
        @if($this->canSelect())
            <th class="p-0 text-left text-black bg-neutral-50 dark:text-white dark:bg-neutral-800">
                <input type="checkbox" wire:model.live="selectedPage" class="w-4 h-4 mx-3">
            </th>
        @endif
    ...
    @if($this->deferLoading && ! $this->initialized)
        <tr class="group">
            <td class="p-0" colspan="{{ $table['columns']->count() + 1 }}">
                <span class="block py-20 text-lg text-center text-black bg-white dark:bg-neutral-900 dark:text-white">
                    @lang('Obteniendo registros...')
                </span>
            </td>
        </tr>
    @else
            ...
            >
                @if($this->canSelect())
                    <td class="p-0"
                        ...
                        <div class="mx-3">
                            <input type="checkbox" wire:model.live="selected" value="{{ $item->getKey() }}" class="w-4 h-4">
                        </div>
                    </td>
                @endif
                @foreach($table['columns'] as $column)
                    @continue(! in_array($column->code(), $this->columns))
                    <td style="@if(isset($item) && isset($item->estado) && $item->estado == 0) background-color: #e6ccca; @endif" 
                        class="p-0"
                        ...
                        @endif
                        x-bind:class="~selected.indexOf('{{ $item->getKey() }}')
                                ? 'cursor-pointer bg-blue-100 group-odd:bg-blue-100 group-hover:bg-blue-200 dark:bg-blue-900 dark:group-odd:bg-blue-900 dark:group-hover:bg-blue-800'
                                : 'cursor-pointer bg-blue-100 group-odd:bg-white group-hover:bg-neutral-200 dark:bg-neutral-800 dark:group-odd:bg-neutral-900 dark:group-hover:bg-neutral-700'">
                        {{ $column->render($item)}}
                    </td>
                @endforeach
            </tr>
