...
class ListaProductoTable extends LivewireTable
{
    protected string $model = ProductosModel::class;

    protected function columns(): array
    {
        return [
            Column::make(__('Acciones'), function (mixed $value): string {
                return '<button type="button" wire:click="editarProducto(' . $value->id . ')" class="px-3 py-2 mb-2 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 me-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"><i class="fa-solid fa-pen-to-square"></i></button>';
            })->asHtml(),
            ...
            Column::make(__('Tipo de impuesto'), function ($value) {
                $tipo_impuesto = TipoAfectacionImpuestoModel::find($value->tipo_impuesto);
                if ($tipo_impuesto) {
                    return $tipo_impuesto->codigo . " - " . $tipo_impuesto->descripcion;
                } else {
                    return 'Sin tipo de impuesto';
                }
            })->sortable()->searchable(),
    ...
    {
        return redirect()->route('editar-producto', ['id' => $id]);
    }
